...
// ---- Event collection (auto-generated by scanner via registry) ----

QJsonObject collectCheckpoint(void *adapter, const ProtocolRegistry &reg)
{
    char *jsonStr = reg.collect_json(adapter);
    QJsonArray events = QJsonDocument::fromJson(jsonStr).array();
    free(jsonStr);

    // Wrap in expected checkpoint format
    for (int i = 0; i < events.size(); ++i) {
        QJsonObject ev = events[i].toObject();
        ev["object"] = QString::fromUtf8(reg.protocol_name);
        events[i] = ev;
    }
    return QJsonObject{
