...
static int read_server_state(struct output_power_server_state *state)
{
    ...
    return protocol_test_invoke_server(output_power_read_server_state, state);
}
...
static int invalid_mode_disconnects(const char *socket_name)
{
    ...
    if (!protocol_test_connect(&connection, socket_name))
        return 1;
    output = protocol_test_bind(&connection, "wl_output", &wl_output_interface, 1);
    manager = protocol_test_bind(&connection, "zwlr_output_power_manager_v1",
