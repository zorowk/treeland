...
static int read_server_state(struct output_management_server_state *state)
{
    ...
    return protocol_test_invoke_server(output_management_read_server_state, state);
}
...
int protocol_test_run(const char *socket_name)
{
    ...
    struct output_management_server_state state;
    int result = 1;

    if (!protocol_test_connect(&connection, socket_name))
        return 1;

    manager = protocol_test_bind(&connection, "zwlr_output_manager_v1",
    ...
    if (wl_display_roundtrip(connection.display) < 0 || !events.head || !events.manager_done_count
        ...
        || !events.mode_size_count || !events.mode_refresh_count)
        goto done;
    ...
    if (wl_display_roundtrip(connection.display) < 0 || events.configuration_succeeded_count != 3
        || events.configuration_failed_count || events.configuration_cancelled_count) {
        zwlr_output_configuration_v1_destroy(configuration);
        goto done;
    }
    ...
    if (!configuration)
        goto done;
    zwlr_output_configuration_v1_apply(configuration);
    if (wl_display_roundtrip(connection.display) < 0 || events.configuration_succeeded_count != 4
        || events.configuration_failed_count || events.configuration_cancelled_count) {
        zwlr_output_configuration_v1_destroy(configuration);
        goto done;
    }
