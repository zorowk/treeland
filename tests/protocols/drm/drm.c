...
static int has_global(const struct protocol_test_connection *connection, const char *interface)
{
...
static int check_initial_events(struct protocol_test_connection *connection,
                                struct wl_drm *drm,
...
static int check_authenticate(struct protocol_test_connection *connection,
                              struct wl_drm *drm,
                              struct drm_events *events)
{
                                        ...
                                        struct protocol_test_connection *connection)
{
    if (!protocol_test_connect(connection, socket_name))
        return NULL;
    return protocol_test_bind(connection, "wl_drm", &wl_drm_interface, 2);
}
...
static int check_create_prime_buffer_dma(const char *socket_name, const char *device)
{
    ...
    const int toplevel_ready = drm
        && protocol_test_xdg_toplevel_create_pending(&connection, &toplevel)
        && wl_display_roundtrip(connection.display) >= 0
        && toplevel.configured
        && protocol_test_xdg_toplevel_ack_latest_configure(&connection, &toplevel);
    ...
    const int sampled = committed
        && protocol_test_invoke_server(drm_read_render_state, &state)
        && state.output_ready
...
int protocol_test_run(const char *socket_name)
{
    ...
    struct wl_drm *drm = protocol_test_bind(&connection, "wl_drm", &wl_drm_interface, 2);
