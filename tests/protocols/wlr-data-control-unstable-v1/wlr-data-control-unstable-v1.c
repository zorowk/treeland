...
struct source_state { unsigned send, cancelled; const char *payload; };
...
static void source_send(void *data, struct zwlr_data_control_source_v1 *source, const char *mime, int32_t fd)
{
    ...
    if (strcmp(mime, "text/plain") == 0)
        write(fd, state->payload, strlen(state->payload));
    close(fd);
    ++state->send;
}
...
static int bind_device(struct protocol_test_connection *connection,
                       ...
                       struct device_state *state)
{
    ...
    *manager = protocol_test_bind(connection, "zwlr_data_control_manager_v1",
...
{
    struct protocol_test_connection source_connection = {0}, target_connection = {0};
    ...
    if (!protocol_test_connect(&source_connection, socket_name)
        || !protocol_test_connect(&target_connection, socket_name)
    ...
    close(pipe_fds[0]);
    if (length != (ssize_t)strlen(first_state.payload) || strcmp(payload, first_state.payload) != 0 || first_state.send != 1) goto failed;
    ...
    protocol_test_disconnect(&target_connection);
    protocol_test_disconnect(&source_connection);
    return 0;
failed:
    protocol_test_disconnect(&target_connection);
    protocol_test_disconnect(&source_connection);
