...
{
    struct protocol_test_connection app = {0};
    struct protocol_test_connection im_connection = {0};
    struct protocol_test_xdg_toplevel toplevel = {0};
    struct input_method_desktop_state desktop = {0};
    ...
    if (!protocol_test_connect(&app, socket_name) || !protocol_test_connect(&im_connection, socket_name))
        goto failed;
    app_seat = protocol_test_bind(&app, "wl_seat", &wl_seat_interface, 1);
    text_manager = protocol_test_bind(&app, "zwp_text_input_manager_v2",
                                      &zwp_text_input_manager_v2_interface, 1);
    virtual_keyboard_manager = protocol_test_bind(&app, "zwp_virtual_keyboard_manager_v1",
                                                  &zwp_virtual_keyboard_manager_v1_interface, 1);
    if (!app_seat || !text_manager || !virtual_keyboard_manager
        || !protocol_test_xdg_toplevel_create(&app, &toplevel))
        goto failed;
    ...
    im_seat = protocol_test_bind(&im_connection, "wl_seat", &wl_seat_interface, 1);
    im_compositor = protocol_test_bind(&im_connection, "wl_compositor", &wl_compositor_interface, 1);
    manager = protocol_test_bind(&im_connection, "zwp_input_method_manager_v2",
    ...
    if (wl_display_roundtrip(im_connection.display) < 0 || first_events.activate
        ...
        goto failed;
    if (!protocol_test_invoke_server(input_method_focus_window, &desktop)
        || !desktop.wrapper_created || !desktop.keyboard_focused)
        goto failed;
    stage = 1;
    ...
    protocol_test_disconnect(&im_connection);
    protocol_test_disconnect(&app);
    return 0;

failed:
    fprintf(stderr, "input-method-v2 failure at stage %d: focused=%d IM=(activate=%u surrounding=%u cause=%u content=%u done=%u unavailable=%u) text=(enter=%u commit=%u delete=%u preedit=%u)\n",
            stage, desktop.keyboard_focused, first_events.activate, first_events.surrounding_text,
            first_events.text_change_cause, first_events.content_type, first_events.done,
