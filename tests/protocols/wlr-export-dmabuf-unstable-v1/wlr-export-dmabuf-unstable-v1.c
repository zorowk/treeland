...
static struct wl_output *bind_output(struct protocol_test_connection *connection)
{
    return protocol_test_bind(connection, "wl_output", &wl_output_interface, 1);
}

static int capture_next_output(struct protocol_test_connection *connection,
                               ...
                               struct wl_output *output, struct export_events *events)
{
    ...
    // capture_output asks wlroots to render one future output frame. The
    // roundtrip lets the production output commit deliver ready or cancel.
    const int dispatched = wl_display_roundtrip(connection->display) >= 0;
...
int protocol_test_run(const char *socket_name)
{
    ...
    struct zwlr_export_dmabuf_manager_v1 *manager = protocol_test_bind(
