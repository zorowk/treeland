...
int protocol_test_run(const char *socket_name)
{
    ...
    /* 3. get_window_control(toplevel) → verify window_id, configure_position, configure_stacking */
    ...
    /* E-level: surface is in parent container, not in always-on-top layer yet */
    if (!read_state(&state) || !state.wrapper_created)
        goto failed;
    ...
    /* 4. set_position(100, 200, 1) → verify configure_position echoes serial=1
     *    E-level: surface's actual QQuickItem position changes */
    ...
    if (last_pos_serial != 1)
        goto failed;
    /* E-level: surface moved to requested position */
    if (!read_state(&state) || state.x != 100 || state.y != 200)
        goto failed;
    stage = 5;

    /* 5. set_z_order(hwnd_topmost, 0) → verify configure_stacking(topmost=1)
     *    E-level: surface is in the always-on-top layer (z=1, effectiveAlwaysOnTop=true) */
    ...
    if (last_topmost != 1)
        goto failed;
    /* E-level: surface is in the always-on-top z layer and effective flag is set */
    if (!read_state(&state) || state.z != 1 || state.effective_always_on_top != 1)
        goto failed;
    stage = 6;

    /* 6. set_z_order(hwnd_notopmost, 0) → verify configure_stacking(topmost=0)
     *    E-level: surface returns to normal layer (z=0, effectiveAlwaysOnTop=false) */
    treeland_wine_window_control_v1_set_z_order(control,
    ...
    if (last_topmost != 0)
        goto failed;
    /* E-level: surface returned to normal z layer */
