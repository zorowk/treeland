...
int protocol_test_run(const char *socket) {
    struct protocol_test_connection c; struct wl_surface *surface=NULL; struct wl_buffer *buffer=NULL; if(!protocol_test_connect(&c,socket))return 1;
    struct wl_compositor *compositor=protocol_test_bind(&c,"wl_compositor",&wl_compositor_interface,1); struct wl_shm *shm=protocol_test_bind(&c,"wl_shm",&wl_shm_interface,1); struct wl_output *output=protocol_test_bind(&c,"wl_output",&wl_output_interface,1); struct zwlr_layer_shell_v1 *shell=protocol_test_bind(&c,"zwlr_layer_shell_v1",&zwlr_layer_shell_v1_interface,5);
    if(!compositor||!shm||!output||!shell)goto fail; surface=wl_compositor_create_surface(compositor); if(!surface)goto fail;
    ...
    if(wl_display_roundtrip(c.display)<0||e.configure!=1||!e.serial)goto fail_layer; zwlr_layer_surface_v1_ack_configure(layer,e.serial); if(!make_buffer(shm,&buffer))goto fail_layer; wl_surface_attach(surface,buffer,0,0); wl_surface_damage(surface,0,0,1920,40); wl_surface_commit(surface); if(wl_display_roundtrip(c.display)<0)goto fail_layer;
    struct layer_shell_server_state state={0}; if(!protocol_test_invoke_server(layer_shell_read_state,&state)||!state.wrapper||!state.container||state.layer!=ZWLR_LAYER_SHELL_V1_LAYER_TOP||state.anchor!=(ZWLR_LAYER_SURFACE_V1_ANCHOR_TOP|ZWLR_LAYER_SURFACE_V1_ANCHOR_LEFT|ZWLR_LAYER_SURFACE_V1_ANCHOR_RIGHT)||state.exclusive_zone!=40||state.top_margin!=2||!state.keyboard_exclusive||!state.focused)goto fail_layer;
    zwlr_layer_surface_v1_set_layer(layer,ZWLR_LAYER_SHELL_V1_LAYER_OVERLAY); wl_surface_commit(surface); if(wl_display_roundtrip(c.display)<0||!protocol_test_invoke_server(layer_shell_read_state,&state)||state.layer!=ZWLR_LAYER_SHELL_V1_LAYER_OVERLAY)goto fail_layer;
    zwlr_layer_surface_v1_destroy(layer); zwlr_layer_shell_v1_destroy(shell); wl_buffer_destroy(buffer); wl_surface_destroy(surface); wl_output_destroy(output); wl_shm_destroy(shm); wl_compositor_destroy(compositor); protocol_test_disconnect(&c); return 0;
fail_layer: zwlr_layer_surface_v1_destroy(layer); if(buffer)wl_buffer_destroy(buffer); if(surface)wl_surface_destroy(surface); fail: if(shell)zwlr_layer_shell_v1_destroy(shell); if(output)wl_output_destroy(output); if(shm)wl_shm_destroy(shm); if(compositor)wl_compositor_destroy(compositor); protocol_test_disconnect(&c); return 1;
}
