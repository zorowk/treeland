...
为使这些状态断言不依赖无头渲染器是否推进 QML 几何动画，desktop fixture 仅在**测试进程**中
将 `Helper.noAnimation` 设为 `true`。生产 `SurfaceWrapper` 在该模式下沿用状态动画
`ready` 阶段的顺序：先向 client 提交 resize，再定位并写入目标 `surfaceState`；因此一次
Wayland roundtrip 后即可读取确定的生产状态。测试不使用任意时长的 sleep、`QTimer` 或无界
事件循环等待。正常桌面进程没有设置该 property，仍保留原有动画行为。
...
`noAnimation` 仅用于把已覆盖的状态转换变成确定性测试，不构成对动画插值或最终帧像素的验证。
