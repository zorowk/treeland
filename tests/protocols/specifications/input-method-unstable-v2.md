...
| focused text-input 的状态快照 | fake app 映射 xdg toplevel、被生产 `Helper::activateSurface()` 聚焦，并 enable text-input-v2，设置 surrounding text/content type/cursor rectangle | fake IM 收到 `activate`、`surrounding_text("abc",3,3)`、`text_change_cause(input_method)`、`content_type(auto_completion,email)` 和一个 `done` | E |
...
测试不依赖系统输入法：一个 client 映射真实 xdg toplevel、创建并 enable text-input-v2，
fixture 通过生产 `Helper::activateSurface()` 聚焦该窗口；另一个 client 创建 IM。由
