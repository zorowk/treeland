...
| [ddm-v1](ddm-v1.md) | I | 客户端连接生命周期 |
| [foreign-toplevel-manager-v1](foreign-toplevel-manager-v1.md) | E | 真实 toplevel、dock preview、窗口状态、激活/焦点与 icon rectangle |
| [input-manager-unstable-v1](input-manager-unstable-v1.md) | I / E（可选） | 确定性 settings/apply；uinput 驱动真实 libinput capability 热插拔 |
| [keyboard-state-notify-unstable-v1](keyboard-state-notify-unstable-v1.md) | P / E | virtual keyboard 驱动真实 seat 的 Caps Lock 状态事件 |
| [output-manager-v1](output-manager-v1.md) | I / P | 真实 `wl_output` 的 primary-output 链路 |
| [personalization-manager-v1](personalization-manager-v1.md) | E / I | 个性化状态挂接到真实 wrapper；font/appearance 配置的 setter/getter 生产回读与恢复 |
| [prelaunch-splash-v2](prelaunch-splash-v2.md) | I / E | splash 请求/关闭信号；生产 splash wrapper 创建、加入 workspace 与销毁 |
| [screensaver-v1](screensaver-v1.md) | E / P | 真实 ext-idle 抑制生命周期 |
| [shortcut-manager-v2](shortcut-manager-v2.md) | E / P | 聚焦窗口捕获与快捷键激活 |
| [virtual-output-manager-v1](virtual-output-manager-v1.md) | P / E | 虚拟输出资源校验；两个既有输出的镜像/恢复 |
| [wallpaper-color-v1](wallpaper-color-v1.md) | I | 订阅、去重与颜色通知 |
| [wallpaper-manager-unstable-v1](wallpaper-manager-unstable-v1.md) | I / P | 真实输出上的壁纸资源生命周期 |
| [wallpaper-shell-unstable-v1](wallpaper-shell-unstable-v1.md) | I / P | wallpaper shell 与 notifier 生命周期 |
| [wallpaper desktop 联合路径](wallpaper-desktop-v1.md) | E | manager 配置、shell surface 与生产 `WallpaperSwitcher` 的绑定 |
| [window-management-v1](window-management-v1.md) | E / P | show-desktop 隐藏并恢复真实窗口 |
| [wine-window-management-unstable-v1](wine-window-management-unstable-v1.md) | P / E | 真实 wrapper 的位置与置顶层同步 |
| [wine-window-state-unstable-v1](wine-window-state-unstable-v1.md) | P / E | 真实 wrapper 的最小化、attention 与可见性同步 |
...
| input-manager-unstable-v1 | 22 / 22 | capability available/unavailable；production apply change mask 与配置值 | 真实 mouse/touchpad 设备配置生效、无设备 failed；uinput E 层需显式启用并实际执行 |
| keyboard-state-notify-unstable-v1 | 6 / 6 | `current_state/state_changed`；virtual keyboard 驱动 Caps Lock | 多 watcher、初始 locked、seat 销毁、重复 apply 与物理键盘对照 |
...
| wallpaper-manager-unstable-v1 | 4 / 5 | `failed/changed`；`set_image_source` 与 `WallpaperSwitcher` 联合路径 | `set_video_source`、媒体解码失败、最终 output 像素 |
| wallpaper-shell-unstable-v1 | 6 / 6 | notifier add/remove、play/pause/slow-down；真实 wallpaper shell 资源生命周期 | `position/set_playback_rate` event payload、媒体播放和最终 output 像素 |
...
分支均已证明。例如 wallpaper 的 image request 已覆盖并有 `WallpaperSwitcher` E 路径，视频
...
新增 XML 及测试时，请从 [_template.md](_template.md) 复制模板，并在同一变更中：
