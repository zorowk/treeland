...
- Fixture：协议 fixture；带 mapped xdg-toplevel 的 desktop fixture，并使用上游 `ext-idle-notify-v1` 客户端。
...
| 场景 | 客户端发送 | 生产业务逻辑与断言 |
| --- | --- | --- |
| 抑制 idle | 创建 privacy-screen manager 后调用 inhibit | 在超时内，真实 ext-idle notification 不发送 `idled` |
| 解除抑制 | 销毁/解除 inhibitor | 同一 ext-idle notification 随后发送 `idled` |
| 非法顺序/对象 | 发送 XML 不允许的请求 | Wayland 连接以规定的 EPROTO/协议错误结束 |

## 已证明的生产链路

客户端首先 map xdg-toplevel，fixture 确认它已成为可见的 workspace `SurfaceWrapper`。
...
在 inhibit 仍存在的 150ms 内，client listener 的 `idled` 计数必须仍为 `0`；这直接
