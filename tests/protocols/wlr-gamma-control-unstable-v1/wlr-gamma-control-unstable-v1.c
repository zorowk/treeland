...
int protocol_test_run(const char *socket_name)
{
    ...
    struct zwlr_gamma_control_manager_v1 *manager = protocol_test_bind(
        &connection, "zwlr_gamma_control_manager_v1", &zwlr_gamma_control_manager_v1_interface, 1);
    struct wl_output *output = protocol_test_bind(&connection, "wl_output", &wl_output_interface, 1);
    if (!manager || !output) {
    ...
    // set_gamma owns the FD even after a failed control has become inert. The
    // native handler must close it without a protocol error before destroy.
    int gamma_pipe[2];
    ...
    close(gamma_pipe[1]);
    zwlr_gamma_control_v1_set_gamma(control, gamma_pipe[0]);
    if (wl_display_roundtrip(connection.display) < 0) {
        zwlr_gamma_control_v1_destroy(control);
        goto failed;
    }
